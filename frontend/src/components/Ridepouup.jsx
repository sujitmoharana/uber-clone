...
const Ridepouup = ({setConfromridePanel,ridepopupPanelref,setRidepopupPanel}) => {
  return (
    <div ref={ridepopupPanelref}  className='bottom-0 absolute bg-white w-full   p-2  '>
      <div className='flex items-center justify-between'>
    <h5 className='text-2xl font-bold'>New Ride is avalaible</h5>
    <h3 className='text-2xl'  onClick={()=>setRidepopupPanel(false)}>
    ...
    }} className='w-full mb-2 bg-green-600 p-2 rounded-lg text-white'>Conform</button>
    <button onClick={()=>{
        setRidepopupPanel(false)
    }} className='w-full mb-2 bg-gray-300 p-2 rounded-lg text-black'>Ignore</button>
   </div>
  )
}

export default Ridepouup